...
import {
  BlockType,
  ExplicitMultipleBlock,
  ExplicitMultipleBlockValue,
} from "~/types";
      ...
      if (item.type === BlockType.Multiple) {
        ...
        if (position !== -1) {
          const moved = arrayMove(
            snippets,
            position,
            dir == "left" ? position - 1 : position + 1
          );
          item.snippets = moved;
          return (draft = obj);
        }
  ...
  return (
    <div className="cursor-auto">
      <div className="mb-3 flex items-center justify-end py-3">
        <Button
          onClick={() => setAddItemModalState((state) => !state)}
          size="sm"
          scheme="success"
        >
          <span className="mr-2">
      ...
      <div className="flex flex-wrap items-center ">
        {activeItems.map((data) => (
          <div
            key={data.name}
            className="card  mr-2 mb-2 bg-base-100 shadow-xl"
          >
